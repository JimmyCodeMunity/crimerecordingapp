...
const AssignRider = () => {
  ...
  const [crimename, setCrime] = useState('');
  ...
  const getOrderDetails = async () => {
    ...
    try {
      const response = await axios.get(
        `https://crime-app-backend.vercel.app/api/v2/admin/crimes/${selector}`,
      );
      ...
      if (crime) {
        setName(crime.name);
        setCrime(crime.crime);
        setLongitude(crime.longitude);
        setLatitude(crime.latitude);
        setAddress(crime.location)
  ...
  const updateOrder = async (e, selectedRider) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://crime-app-backend.vercel.app/api/v2/admin/updatecrime/${selector}`,
        { status: selectedRider },
      );

      console.log(response.data); // Log the response data for debugging
      Swal.fire({
        icon: 'success',
        title: 'Crime updated successfully',
      });
    } catch (error) {
                  ...
                  <div className="mb-5.5">
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        {status === 'none' ? (
                          <option selected>Select crime action</option>
                        ) : (
